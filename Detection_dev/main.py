...
if not os.path.exists(CONFIG_JSON_PATH):
    ...
    subprocess.run(
        [sys.executable, os.path.join(CURRENT_SCRIPT_PATH, "setup.py")], check=True
    )
...
if __name__ == "__main__":
    ...
    print(
        f"{cfg["weather"]["latitude"],cfg["weather"]["longitude"]}\nDate: {cfg["weather"]["date"]}, "
    ...
    out = cv2.VideoWriter(
        OUTPUT_VIDEO_PATH, fourcc, int(fps), (frameWidth, frameHeight)
    )
    ...
    try:
        while cap.isOpened():
            ...
            staleIds = [
                ...
                if carObj.lastSeen < frameIndex - 5
            ]
            ...
            if frameIndex % RADAR_STEP_INTERVAL == 0:
                ...
                if clusterCenters:
                    closestRadarCluster = min(
                        clusterCenters, key=lambda c: abs(c["y_corrected"])
                    )
                    closestRadarDistance = (
                        abs(closestRadarCluster["y_corrected"]) - ACTUAL_RADAR_OFFSET
                    )

                    if (
                        closestRadarDistance <= OVERTAKING_LINE_THRESHOLD
                        and overtakingLineY is None
                    ):
                        overtakingLineTriggered = True

                for cluster in clusterCenters:
                    currentDistance: float = (
                        abs(cluster["y_corrected"]) - ACTUAL_RADAR_OFFSET
                    )
                    currentVelocity: float = abs(cluster["radial_velocity"])
                    stoppingDistance: float = abs(
                        calcStoppingDistance(currentVelocity, multiplier)
                    )
                    if DEBUG:
                        print(
                            f"Distance: {currentDistance:.2f}, adjusted with offset {ACTUAL_RADAR_OFFSET:.1f} m"
                        )
            ...
            if results[0].boxes.id is not None:
                ...
                for boxXyxy, boxXywh, trackId, conf in zip(
                    boxesXyxy, boxesXywh, trackIds, confidences
                ):
                    ...
                    if len(detected_lines) >= 2:
                        ...
                        if None not in (m_left, b_left, m_right, b_right):
                            ...
                            out_of_left = x1 < (
                                x_left_line_at_y2 - LANE_DEPARTURE_MARGIN_PX
                            )
                            out_of_right = x2 > (
                                x_right_line_at_y2 + LANE_DEPARTURE_MARGIN_PX
                            )
                            ...
                            if out_of_left or out_of_right:
                                ...
                                if (
                                    frameIndex
                                    - laneDepartureCooldown.get(
                                        trackId, -LANE_DEPARTURE_COOLDOWN_FRAMES
                                    )
                ...
                if overtakingLineTriggered and overtakingLineY is None:
                    closestCarIdx = max(
                        range(len(trackIds)), key=lambda i: boxesXyxy[i][3]
                    )
                    overtakingLineY = int(boxesXyxy[closestCarIdx][3])
                    print(
                        f"[MONITOR] Overtaking zone starts from Y={overtakingLineY}px ({closestRadarDistance:.2f}m)"
                    )
                ...
                if prevZoneRanking:
                    ...
                    for overtaker, overtaken in overtakes:
                        ...
                        if (
                            frameIndex
                            - overtakeCooldown.get(pair, -OVERTAKE_COOLDOWN_FRAMES)
