...
if __name__ == "__main__":
    ...
    try:
        while cap.isOpened():
            ...
            is_detection_frame = (frameIndex % 1 == 0)
            inference_ms = 0.0
                ...
                for i, (boxXyxy, boxXywh, trackId, conf) in enumerate(zip(boxesXyxy, boxesXywh, trackIds, confidences)):
                    ...
                    if is_detection_frame:
                        ...
                        if len(car.history) >= 2:
                            dx = car.history[-1][0] - car.history[-2][0]
                            dy = car.history[-1][1] - car.history[-2][1]

                            boxXywh[0] += dx
                            boxXywh[1] += dy
                            
                            w, h = boxXywh[2], boxXywh[3]
                            boxXyxy[0] = boxXywh[0] - w/2
                            boxXyxy[1] = boxXywh[1] - h/2
                            boxXyxy[2] = boxXywh[0] + w/2
                            boxXyxy[3] = boxXywh[1] + h/2
            ...
            num_detections = len(last_results_boxes) if last_results_boxes is not None else 0
            avg_confidence = float(np.mean(last_results_boxes.conf.cpu().numpy())) if num_detections > 0 else 0.0
