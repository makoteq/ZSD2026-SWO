...
from utils.depth_metric import DepthMetric, rankCarsByDepth, fillBboxesRowMax, fillBboxesRowMeanBackground, flattenRowsMedianBackground, saveDepthVisualization
...
DEPTH_MODEL_PATH = os.path.join(DATA_DIR, "models", "depth_anything_v2_metric_vkitti_vits.pth") # NEW model to metric depth
...
if __name__ == "__main__":
    ...
    ## --- generally useless, saves png of the first frame for the reference for the depth map
    ...
    ## Metric Depth Map for the first frame -----
    cap.set(cv2.CAP_PROP_POS_FRAMES, int(START_TIME * fps))
    _, firstFrame = cap.read()
    cap.set(cv2.CAP_PROP_POS_FRAMES, int(START_TIME * fps))

    if os.path.exists(NPY_PATH):
        print("DepthMetric: loaded depth map from base_depth.npy file.")
        baseDepthMap = np.load(NPY_PATH)
    else:
        depthProcessor = DepthMetric(modelPath=DEPTH_MODEL_PATH, libPath=DEPTH_LIB_PATH)
    ...
    ## CHOOSE one of the 2 func below for filling the bboxes of detected cars in the depth map of the first frame
    # baseDepthMap = fillBboxesRowMax(baseDepthMap, firstFrameBboxes, paddingFactor=0.05)
    # baseDepthMap = fillBboxesRowMeanBackground(baseDepthMap, firstFrameBboxes, paddingFactor=0.35)
    ...
    try:
        while cap.isOpened():
            ...
            if results[0].boxes.id is not None:
                ...
                ## Build list of detected cars in the current frame and rank them, less depth value (m) means closer to the camera 
                currentFrameCars = [
                ...
                ranked_display = [{'id': r['id'], 'depth_m': round(r['depth_m'], 2)} for r in ranked]
                print(f"Frame {frameIndex} | ranking: {ranked_display}")
