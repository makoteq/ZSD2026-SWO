...
def brake_to_stop(vehicle, steer = 0.0):
    """Brake the car until complete stop.
    Dynamically slows down the car.
    Args:
        vehicle (carla.Actor): actor car that will stop.
        steer (float): vehicle continues on its path while stoping.
    Returns:
        None
    """
    v = speed_kph(vehicle)
...
def run(world, blueprint_library, duration_sec=30.0, output_dir=None):
    """Run a pull-over anomaly scenario with a single vehicle.

    Spawns one car that cruises at a random speed, then pulls toward the
    road shoulder but never brakes ensuring that it will leave camera view completely.
    The scenario progresses through three phases: cruise, pull and straighten.
